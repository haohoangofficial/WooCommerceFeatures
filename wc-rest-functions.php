<?php
/**
 * Replace file follow path wp-content\plugins\woocommerce\includes\wc-rest-functions.php
 */
/**
...
function wc_rest_upload_image_from_url( $image_url ) {
    // GG Drive format https://drive.google.com/uc?id=IDIMAGE
 ...
 */
function wc_rest_set_uploaded_image_as_attachment( $image_url, $id = 0 ) {
	$host = parse_url($image_url, PHP_URL_HOST);
	if($host == 'drive.google.com') {
		...
		if($attachment_id) {
			...
			$attachment = array(
				...
				'post_parent'    => $id,
				'post_title' => explode("uc?id=",$image_url)[1],
			);
